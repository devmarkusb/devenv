...
include(${CMAKE_CURRENT_LIST_DIR}/detail/third-party-warnings.cmake)

option(MB_DEVENV_ENABLE_LTO "enables link time optimization" OFF)

if("${CMAKE_CXX_COMPILER_ID}" STREQUAL "Clang")
    set(MB_DEVENV_IS_NON_APPLE_CLANG_COMPILER TRUE)
else()
    set(MB_DEVENV_IS_NON_APPLE_CLANG_COMPILER FALSE)
endif()

option(
    MB_DEVENV_USE_LLD_LINKER
    "use lld linker"
    ${MB_DEVENV_IS_NON_APPLE_CLANG_COMPILER}
)

# If requested, verify the toolchain can link with lld before setting -fuse-ld=lld.
set(MB_DEVENV_HAVE_WORKING_LLD FALSE)
if(MB_DEVENV_USE_LLD_LINKER)
    if(CMAKE_CXX_COMPILER)
        set(_ul_lld_probe_src
            "${CMAKE_BINARY_DIR}/CMakeFiles/mb_devenv_lld_probe.cxx"
        )
        file(WRITE "${_ul_lld_probe_src}" "int main(){return 0;}\n")
        try_compile(
            MB_DEVENV_HAVE_WORKING_LLD
            "${CMAKE_BINARY_DIR}"
            "${_ul_lld_probe_src}"
    ...
    elseif(CMAKE_C_COMPILER)
        set(_ul_lld_probe_src
            "${CMAKE_BINARY_DIR}/CMakeFiles/mb_devenv_lld_probe.c"
        )
    ...
    if(NOT MB_DEVENV_HAVE_WORKING_LLD)
        message(
            STATUS
            "MB_DEVENV_USE_LLD_LINKER is ON but linking with -fuse-ld=lld failed; using the default linker"
        )
    endif()
endif()
...
# MB_DEVENV_BITS will be 32, 64, ...
math(EXPR MB_DEVENV_BITS "8 * ${CMAKE_SIZEOF_VOID_P}")

# MB_DEVENV_NPROC will contain processor count and 0 if count couldn't be determined
include(ProcessorCount)
ProcessorCount(MB_DEVENV_NPROC)
...
if(MB_DEVENV_HAVE_WORKING_LLD)
    set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} -fuse-ld=lld")
    set(CMAKE_MODULE_LINKER_FLAGS "${CMAKE_MODULE_LINKER_FLAGS} -fuse-ld=lld")
    set(CMAKE_SHARED_LINKER_FLAGS "${CMAKE_SHARED_LINKER_FLAGS} -fuse-ld=lld")
endif()

if(MB_DEVENV_ENABLE_LTO)
...
# When ON, mb_devenv_set_target_warnings() applies the Clang -Weverything set to every target (see macro). Per-call
# override: mb_devenv_set_target_warnings(t ALL_WARNINGS) or ... NO_ALL_WARNINGS (last keyword wins).
option(
    MB_DEVENV_ALL_WARNINGS
    "Clang: -Weverything (+ tuned -Wno*) on each mb_devenv_set_target_warnings target, not global CXX flags"
    OFF
)

macro(mb_devenv_set_target_warnings target)
    set(_mb_devenv_tw_all ${MB_DEVENV_ALL_WARNINGS})
    foreach(_mb_devenv_tw_arg ${ARGN})
        if(_mb_devenv_tw_arg STREQUAL "ALL_WARNINGS")
            set(_mb_devenv_tw_all TRUE)
        elseif(_mb_devenv_tw_arg STREQUAL "NO_ALL_WARNINGS")
            set(_mb_devenv_tw_all FALSE)
        endif()
    endforeach()

    if(_mb_devenv_tw_all)
        if("${CMAKE_CXX_COMPILER_ID}" STREQUAL "MSVC")
            message(
                FATAL_ERROR
                "MB_DEVENV_ALL_WARNINGS / ALL_WARNINGS not implemented for MSVC compiler"
            )
        endif()
    endif()

    get_target_property(_mb_devenv_tw_type ${target} TYPE)
    if(NOT _mb_devenv_tw_type STREQUAL "INTERFACE_LIBRARY")
        if(
...
macro(mb_devenv_set_target_essentials target)
    if(WIN32)
...
macro(mb_devenv_set_target_defaults target)
    mb_devenv_set_target_essentials(${target})

    set_target_properties(${target} PROPERTIES VERIFY_INTERFACE_HEADER_SETS ON)

    mb_devenv_set_target_warnings(${target})

    if(NOT MB_DEVENV_ANDROID) # easier than to fix the follow-up processes
    ...
    if("${MB_DEVENV_DEPLOY_TARGET}" STREQUAL "uwp")
...
macro(mb_devenv_set_target_filesystem target)
    if(MB_DEVENV_HAS_CPP_FILESYSTEM)
        if("${MB_DEVENV_CPP_STD_LIB}" STREQUAL "libstdc++")
...
macro(mb_devenv_set_target_pthread target)
    if(MB_DEVENV_LINUX)
        target_link_libraries(${target} PUBLIC pthread)
    endif()
endmacro()

macro(mb_devenv_set_target_cuda_separable_compilation target)
    set_target_properties(${target} PROPERTIES CUDA_SEPARABLE_COMPILATION ON)
endmacro()

# shared lib export define (such that the build know whether to export or import symbols)
macro(mb_devenv_set_target_lib_export_def target define)
...
macro(mb_devenv_target_coverage target)
